...
function w = dtc(a, crit, chisqstopval, prune, t, nfeat)
  ...
  global FIRST; % workaround for too early stopping    
  ...
  % When no input data is given, an empty tree is defined:
    ...
    w = prmapping(mfilename, {crit, chisqstopval, prune, t, nfeat});
  ...
  elseif (nargin == 2) && ismapping(crit)
    ...
    FIRST = true; % calling training for the first time
    ...
    tree = makedt( ...
      +a, feattype, usefeat, nfeat, ...
      c, nlab, weights, cost, crit, chisqstopval);
    
    if ~isempty(prune)
      if strcmpi(prune, 'prunep')
        if ~isempty(cost)
          msg = [ ...
            'Pessimistic prunning based on ' ...
            'misclassification costs is not implemented'];
          error(msg);
        end
        tree = prunep(tree);
    ...
    tree.featlab = getfeatlab(a);
    tree.featdom = getfeatdom(a);    
        
    w = prmapping(mfilename, 'trained', tree, getlablist(a), k, c);      
    w = setname(w, ['DecTree' upper(crit)]);
    w = setcost(w, cost);
  end
...
%          node J if it is considered as a leaf.
%
...
%
%          .ERRT Tree error. ERRT(J) is the misclassification error (cost) 
%          on training samples which reached the node J if this node is
...
%          for the leaf, otherwise it is > 1). For continues features, 
%          if LENGTH(FVAL{J}) == 1 then it contains a
%          threshold THR for splitting into the left (<= THR) and the
%          right (> THR) branches. Some such nodes may contain the third
%          branch for non-applicable values. If FVAL{J} == [] and the node 
%          J is not a leaf then it means that split is perfromed only 
%          between applicable and non-applicable values. 
%         
%          .NIDX Branch node indices. For nominal features NIDX{J}(K) is
%          the index of branch node of node J with value FVAL{J}(K)
%          of feature FIDX(J). For continues features, if 
%          LENGTH(FVAL{J}) == 1 then NIDX{J}(1) is the index of the left 
%          branch node, NIDX{J}(2) is the index of the right branch node 
%          and NIDX{J}(3), if present, is the index of the non-applicable 
%          branch node. If FVAL{J} == [] and node J is not a leaf then 
%          NIDX{J}(1) is the index of branch node containing objects with 
%          applicable values of feature FIDX(J) and NIDX{J}(2) is the index 
%          of branch node containing objects with non-applicable values of
%          the same feature.
% 
...
% 
%  [FIDX, FVAL, NB, BIDX, CHISQVAL, CRITVAL] = FINDSPLIT(A, FEATTYPE, USEFEAT, NFEAT, C, NLAB, WEIGHTS, COST, CRIT)
% 
    ...
    switch feattype(fidx)
      case 0 % continous/ordered feature
        ...
        for j = 1:c
          ...
          AP(j) = sum(wap(J));
          if (ns > 0) && (mj > 0)
            %L(:, j) = (repmat(splitlowidx, [1, mj]) >= repmat(J.', [ns, 1]))*wap(J);
            L(:, j) = bsxfun(@ge, splitlowidx, J.') * wap(J);
            R(:, j) = AP(j) - L(:, j);
          end
        end
        ...
        if ~isnan(cv) && (cv > -inf) 
          if (ns > 0) && ~isempty(i)
            ...
            if nna == 0 
              ...
              % df = (2-1) * (c-1) 
              chisqval(f) = gammainc(0.5 * chisqval(f), 0.5 * (c-1)); 
            else
              KNA = K*(MNA/MK);
              KL = K*(ML(i)/MK);
              KR = K*(MR(i)/MK);
              ...
              % df = (3-1) * (c-1) 
              chisqval(f) = gammainc(0.5 * chisqval(f), c - 1);              
            end
          else
            fval{f} = [];
            KNA = K*(MNA/MK);
            KAP = K*(MAP/MK);
            chisqval(f) = sum(((NA - KNA).^2)./KNA + ((AP - KAP).^2)./KAP);
            % df = (2-1) * (c-1) 
            chisqval(f) = gammainc(0.5 * chisqval(f), 0.5 * (c-1));
          end
        end
      ...
      case 1 % nominal feature
        ...
        for j=1:c
          ...
          if mj > 0
            ...
            B(:, j) = bsxfun(@eq, vf, af(J).') * weights(J);
          end
        end
        ...
        if ~isnan(cv) && (cv > -inf)
          ...
          % df = (v-1) * (c-1)
          chisqval(f) = gammainc(0.5 * chisqval(f), 0.5 * (v-1) * (c-1));
          fval{f} = vf.';
        end
    end
  end
...
function [critval, idx] = igr(feattype, varargin) %#ok<DEFNU>
	
  global FIRST; % work around too early stopping
  
  switch feattype
    case 0
      [MU, MK, K, MNA, NA, MAP, AP, ML, L, MR, R, cost, weights, uv, sli] = deal(varargin{:}); %#ok<NASGU,ASGLU>
      ...
      if ~isempty(R) 
        infol = - sum(L .* log2(bsxfun(@rdivide, L, ML)), 2);
        infor = - sum(R .* log2(bsxfun(@rdivide, R, MR)), 2);
        infolr = infol + infor;
    ...
    case 1
      idx = [];
      [MU, MK, K, MB, B, cost, weights] = deal(varargin{:}); %#ok<NASGU,ASGLU>
      ...
      infonew = - sum(sum(B .* log2(bsxfun(@rdivide, B, MB)), 2), 1);
...
function [critval, idx] = gini(feattype, varargin) %#ok<DEFNU>
...
%           );  
          ...
          puritylr = ( ...
            sum(L .* bsxfun(@rdivide, L, ML), 2) + ...
            sum(R .* bsxfun(@rdivide, R, MR), 2));  
...
%             sum((R*cost) .* (R./(repmat(MR, [1 size(R, 2)]))), 2);
          impuritylr = ...
            sum((L*cost) .* bsxfun(@rdivide, L, ML), 2) + ...
            sum((R*cost) .* bsxfun(@rdivide, R, MR), 2);
    ...
    case 1
      idx = [];
      [MU, MK, K, MB, B, cost, weights] = deal(varargin{:}); %#ok<NASGU>
      
      if isempty(cost)
        purityold = K * (K.'/MK);
        %impurityold = MK - purityold;        
%         puritynew = sum(sum(B .* (B./(repmat(MB, [1 size(B, 2)]))), 2));
        puritynew = sum(sum(B .* bsxfun(@divide, B, MB), 2));
        %impuritynew = MK - puritynew;        
        deltaimpurity = puritynew - purityold;
...
%         );
        ...
        impuritynew = sum(sum((B*cost) .* bsxfun(@divide, B, MB), 2), 1);
        deltaimpurity = impurityold - impuritynew;
      end
  end
  ...
  if deltaimpurity > deltamin || FIRST
...
function [critval, idx] = miscls(feattype, varargin) %#ok<DEFNU>
...
function tree = prunet(tree, t, cost)
  ...
  if (nargout ~= 0) || (nargin ~= 1) || ~isscalar(tree) || ~isnumeric(tree) || ~isint(tree)
    ...
    pt = tree;
    mt = size(pt.post,1);
    [p, s] = mapdt(pt, +t, getnlab(t), weights); %#ok<ASGLU>
