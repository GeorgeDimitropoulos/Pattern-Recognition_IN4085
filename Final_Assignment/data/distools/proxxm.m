...
if (isstr(type))
  ...
  [m,k] = size(A);
  if isdataset(A)
    W = prmapping(mfilename,'trained',{A,type,s,weights},getlab(A),getfeatsize(A),getobjsize(A));
  else
...
elseif ismapping(type)
  ...
  switch type
    ...
    case {'exp-sum','es'}
     ...
     case {'shiftdist'} % not properly implemented for test objects
        D = dist2(B,A);
        if s(1) ~= 2
          D = sqrt(D).^s(1);
        end
        D = -D;
				n = size(D,1);
        % shift data such that mean lies in the origin
        H = -repmat(1/n,n,n); 
        H(1:n+1:end) = H(1:n+1:end) + 1;  % H = eye(n) - ones(n,n)/n
				size(D)
				size(H)
        D = -0.5 * H * D * H;             % D is now the matrix of inner products
