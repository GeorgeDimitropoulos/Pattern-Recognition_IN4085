...
if nargin < 1 | isempty(d),
  W = prmapping(mfilename,{R,alf,prec});
...
if (isdataset(d) | isa(d,'double'))
  if ismapping(R)
    ...
    if isdataset(d),
      W = prdataset(W,getlab(d),'prior',getprior(d));
...
if ismapping(d)
  ...
  W = prmapping(mfilename,'trained',{R,Q,L,pars{4},pars{5}},[],m,length(J));
  W = setname(W,'Spherical embedding');
  return
end
...
W = prmapping(mfilename,'trained',{R,Q,L,sig,prec},[],m,sum(sig));
W = setname(W,'Spherical  embedding');
return
